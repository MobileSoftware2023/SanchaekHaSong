...
exports.calculateAndNotifyRankings = functions.pubsub.schedule('40 3 * * 2') // 매주 월요일 자정
    .timeZone('Asia/Seoul')
    .onRun(async (context) => {
        try {
            //@ranking_college 초기화
...
exports.resetWalkCounts = functions.pubsub.schedule('40 3 * * 2') // 매주 월요일 자정
    .timeZone('Asia/Seoul')
    .onRun(async (context) => {
        try {
...
exports.resetDailyQuest = functions.pubsub.schedule('40 3 * * *') // 매일 자정
    .timeZone('Asia/Seoul')
    .onRun(async (context) => {
        try {
            // 일일 퀘스트 초기화
